...
public class GameController {
    ...
    private static boolean move(TerrainTileFX newTile) {
        ...
        if (!result) {
            ...
            alert.setHeaderText("Can't Move");
            alert.setContentText("You cannot move onto the selected tile.");
    ...
    private static void attackTile(TerrainTile attacker, TerrainTile enemy) {
        ...
        if (enemy.isEmpty()
            ...
            alert.setTitle("Error!");
            alert.setHeaderText("Can't Perform Attack");
            alert.setContentText("You either tried to attack an empty cell"
                                 + " or your own troop, or your unit cannot"
                                 + " attack this turn or the enemy isn't"
                                 + " adjacent to your unit.");
            alert.initOwner(CivilizationGame.getStage());
            alert.showAndWait();
            return;
        }
        boolean tmp = attacker.getOccupant().getOwner() == playerCivilization;
        alert = new Alert(AlertType.INFORMATION);
        alert.setTitle(tmp ? "Congratulations!" : "Beware!");
        alert.setHeaderText("Attack performed");
        alert.setContentText(tmp ? "You attacked an enemy unit."
            : "Enemy Attcked You.");
        alert.initOwner(CivilizationGame.getStage());
        alert.showAndWait();
        ((MilitaryUnit) attacker.getOccupant()).attack(enemy.getOccupant());

        if (attacker.getOccupant().isDestroyed()) {
            alert.setHeaderText(tmp ? "Oh no!" : "Enemy Destroyed!");
            alert.setContentText(tmp ? "You were destroyed!" : null);
            alert.showAndWait();
            attacker.setOccupant(null);
        }

        if ((enemy.getOccupant()).isDestroyed()) {
            alert.setHeaderText(tmp ? "Enemy Destroyed!" : "Oh no!");
            alert.setContentText(tmp ? null : "You were destroyed!");
            alert.showAndWait();
